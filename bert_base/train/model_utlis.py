...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    embedding = model.get_sequence_output()#前面把bert模型引进进来，抽取最后一层
...
def filed_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):#每一个样本要转化成feature。
        ...
        def create_int_feature(values):#因为tf_recode中只有几种数据类型，要转化过来，这里转化成int64类型
        ...
        features["input_mask"] = create_int_feature(feature.input_mask)#训练bert的方式
        features["segment_ids"] = create_int_feature(feature.segment_ids)
        features["label_ids"] = create_int_feature(feature.label_ids)
        tf_example = tf.train.Example(features=tf.train.Features(feature=features))#tf_recode标准格式
        writer.write(tf_example.SerializeToString())#序列化到硬盘上

def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):#读取方法
    name_to_features = {
    ...
    def _decode_record(record, name_to_features):#主要是对编码类型进行转化
